...
namespace Bloknot
{
    public partial class Form1 : Form
    {
        ...
        private void openToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            // Помилка із завантаження - даний формат файлу не підтримується...
            // Жодне з можливих рішень не підійшли, можливо проблема при збереженні самого файлу, хоча зберігає стандартним методом
      ...
      open.FileName.Length > 0)
            {
                ...
                if (!rewrite)
                {
                    //AddTab(open.FileName, sRtfFile);
                    
                }
                else
                {
                   
                    GetTextBox().LoadFile(open.FileName, RichTextBoxStreamType.RichText);
                    //GetTextBox().Rtf = sr.ReadToEnd();
                    maxSymbols.Value = GetTextBox().Text.Length;
        ...
        private void saveToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (GetTabName() == "No name" || !File.Exists(GetTabName()))
            {
                   ...
                   save.FileName.Length > 0)
                {
                    ...
                    GetTextBox().SaveFile(save.FileName, RichTextBoxStreamType.RichText);
                    ChangeTabName(save.FileName);
                }
            }
            else
            {
                GetTextBox().SaveFile(GetTabName(), RichTextBoxStreamType.RichText);
            }
        }
